...
func (c *Controller) Start() error {
	...
	if newNodeInfo.Port == 0 {
		return errors.New("服务器端口号必须大于 0")
	}
...
func (c *Controller) nodeInfoMonitor() (err error) {
	...
	if nodeInfoChanged {
		...
		c.logger.Printf("删除了 %d 个用户, 添加了 %d 个用户", len(deleted), len(added))
	}
...
func (c *Controller) userInfoMonitor() (err error) {
	...
	if c.config.AutoSpeedLimitConfig.Limit > 0 && len(c.limitedUsers) > 0 {
		...
		for user, limitInfo := range c.limitedUsers {
			if time.Now().Unix() > limitInfo.end {
				...
				c.logger.Printf("用户: %s 限速: %d 结束时间: nil (已解除限速)", c.buildUserTag(&user), user.SpeedLimit)
				delete(c.limitedUsers, user)
			} else {
				c.logger.Printf("用户: %s 限速: %d 结束时间: %s", c.buildUserTag(&user), limitInfo.currentSpeedLimit, time.Unix(c.limitedUsers[user].end, 0).Format("01-02 15:04:05"))
			}
		}
	...
	if len(userTraffic) > 0 {
		c.logger.Printf("用户流量: %v", userTraffic)
		if len(upCounterList) > 0 {
			for i, upCounter := range upCounterList {
				c.logger.Printf("上传计数器 %d: %v", i, upCounter)
			}
		}
		
		if len(downCounterList) > 0 {
			for i, downCounter := range downCounterList {
				c.logger.Printf("下载计数器 %d: %v", i, downCounter)
			}
		}
	...
	if onlineDevice, err := c.GetOnlineDevice(c.Tag); err != nil {
		...
		if err = c.apiClient.ReportNodeOnlineUsers(onlineDevice); err != nil {
			...
			c.logger.Printf("上报 %d 个在线用户", len(*onlineDevice))
		}
	}
	...
	if detectResult, err := c.GetDetectResult(c.Tag); err != nil {
		...
		if err = c.apiClient.ReportIllegal(detectResult); err != nil {
			...
			c.logger.Printf("上报 %d 个违规行为", len(*detectResult))
		}
