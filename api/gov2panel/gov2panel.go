...
func (c *APIClient) parseResponse(res *resty.Response, path string, err error) (*simplejson.Json, error) {
	if err != nil {
		return nil, fmt.Errorf("请求 %s 失败: %v", c.assembleURL(path), err)
	}

	if res.StatusCode() > 399 {
		return nil, fmt.Errorf("请求 %s 失败: %s, %v", c.assembleURL(path), res.String(), err)
	}

	rtn, err := simplejson.NewJson(res.Body())
	if err != nil {
		return nil, fmt.Errorf("响应 %s 无效", res.String())
	}

	return rtn, nil
}

// GetNodeInfo 从面板获取 NodeInfo 配置
func (c *APIClient) GetNodeInfo() (nodeInfo *api.NodeInfo, err error) {
	...
	if gconv.Uint32(server.Port) == 0 {
		return nil, errors.New("服务器端口必须大于 0")
	}
	...
	default:
		return nil, fmt.Errorf("不支持的节点类型：%s", c.NodeType)
	}

	if err != nil {
		return nil, fmt.Errorf("解析节点信息失败：%s, \n错误：%v", res.String(), err)
	}
...
// ReportUserTraffic reports the user traffic 报告用户流量
func (c *APIClient) ReportUserTraffic(userTraffic *[]api.UserTraffic) error {
	path := "/api/server/push"

	userTrafficJSON, _ := json.MarshalIndent(userTraffic, "", "  ")
	fmt.Printf("流量报备: %s\n", string(userTrafficJSON))
